...
export default function SuperAdminActivityPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-[var(--text-secondary)]">Every action across the entire platform ({logs.length} events)</p>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select value={actionFilter} onChange={(e) => setActionFilter(e.target.value)} className="h-10 rounded-md border bg-[var(--surface)] px-3 py-2 text-sm">
        ...
        <Card>
          <CardContent className="p-0">
            <div className="divide-y max-h-[600px] overflow-y-auto">
              {filtered.map((log) => (
                <div key={log.id} className="p-4 flex items-start gap-3 hover:bg-[var(--surface-subtle)]">
                  <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${actionColors[log.action] || "bg-[var(--surface-accent)] text-[var(--text-primary)]"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {log.entity_name && <p className="text-sm text-[var(--text-secondary)] mt-1">{log.entity_name}</p>}
                  </div>
                  <p className="text-xs text-[var(--text-tertiary)] shrink-0">{formatDateTime(log.created_at)}</p>
                </div>
              ))}
            </div>
          </CardContent>
