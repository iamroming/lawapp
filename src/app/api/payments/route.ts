...
export async function GET(request: NextRequest) {
  ...
  if (isOwner && firmId) {
    ...
    query = query.eq("received_by", user.uuid);
  }
...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await supabase
    .from("payments")
    .insert({ ...parsed.data, received_by: user.uuid, firm_id: profile?.firm_id || null, client_id: parsed.data.client_id || null, case_id: parsed.data.case_id || null })
    .select()
    .single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
