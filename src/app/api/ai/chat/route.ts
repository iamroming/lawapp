...
const SYSTEM_PROMPT = `You are LawXP AI, a legal assistant specializing in Indian law. You help lawyers with:
...
export async function POST(request: NextRequest) {
  ...
  const { allowed } = await checkRateLimit(`ai-chat:${user.id}:${ip}`, {
    windowMs: 3600000,
    maxRequests: 20,
  });
  if (!allowed) {
    ...
    if (!AI_API_KEY) {
      return NextResponse.json({
        data: {
          role: "assistant",
          content:
            "AI service is not configured. Please set the AI_API_KEY environment variable to enable the AI chatbot.",
        },
      });
    }
