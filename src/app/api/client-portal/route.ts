...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: client } = isOwner && profile?.firm_id
      ? await supabase.from("clients").select("id, full_name, email, phone, firm_id").eq("id", client_id).eq("firm_id", profile.firm_id).single()
      : await supabase.from("clients").select("id, full_name, email, phone, created_by").eq("id", client_id).eq("created_by", user.uuid).single();
    ...
    const { data: portalUser, error } = await supabase.from("client_portal_users").insert({
      client_id, user_id: authUser.user.id, email: portalEmail, role: "client", firm_id: profile?.firm_id || null,
    }).select().single();
    ...
    try {
      ...
      if (resendKey) {
        ...
        const firmName = escapeHtml(user.email?.split("@")[0] || "Your Law Firm");
        const clientName = escapeHtml(client.full_name);
        await fetch("https://api.resend.com/emails", {
          method: "POST",
          headers: { "Authorization": "Bearer " + resendKey, "Content-Type": "application/json" },
          body: JSON.stringify({
            from: "CaseFiles <noreply@" + (process.env.EMAIL_DOMAIN || "CaseFiles.in") + ">",
            to: portalEmail,
            subject: "You're invited to " + firmName + " Client Portal",
            html: "<h2>Welcome to " + firmName + " Client Portal</h2><p>Dear " + clientName + ",</p><p>You have been granted access to the client portal of " + firmName + ".</p><p>You can now log in to view your cases, documents, invoices, and communicate with your lawyer.</p><p><strong>Login URL:</strong> " + (process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000") + "/client-login</p><p>Your login email: " + portalEmail + "</p><p>Please set your password on first login.</p><br/><p>Best regards,<br/>" + firmName + "</p>",
          }),
        });
      }
    } catch (emailError) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Owners/partners see all firm portal users; others see only their created clients
    const { data: clients } = isOwner && profile?.firm_id
      ? await supabase.from("clients").select("id").eq("firm_id", profile.firm_id)
      : await supabase.from("clients").select("id").eq("created_by", user.uuid);
