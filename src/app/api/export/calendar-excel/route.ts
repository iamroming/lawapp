...
import { generateCalendarExcel } from "@/lib/calendar-export";
...
export async function GET(req: NextRequest) {
  ...
  const buffer = generateCalendarExcel(events, monthYear);

  return new NextResponse(buffer, {
    headers: {
      "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      "Content-Disposition": `attachment; filename="casefiles-calendar-${month || "all"}-${year || "all"}.xlsx"`,
    },
  });
}
